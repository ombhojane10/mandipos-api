...
export class AccountsService {
  ...
  async createShop(p: Principal, shop: { name: string; mandi: string; shopNo: string; phad?: string; gstin: string; role: string; ownerName: string }):
    ...
    return this.db.tx(async (tx) => {
      ...
      const shopId = uuidv7();
      await tx.query(
        `INSERT INTO shops (id, name, mandi, shop_no, phad, gstin, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [shopId, shop.name, shop.mandi, shop.shopNo, shop.phad ?? '', shop.gstin, p.userId],
      );
